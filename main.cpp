...
int main()
{
    ...
    for (int el = 0; el < enriched_elements.heaviside_enriched.size(); ++el)
    {   
        break;
        const HeavisideEnriched &hvsd_enr =
            enriched_elements.heaviside_enriched[el];

        const std::array<int, 4> &element =
            mesh.elements[hvsd_enr.id];

        const HeavisideTriangulation &hvsd_trng =
            enriched_elements_triangulation
                .heaviside_enriched_triangulation[el];

        // -------------------------------
        // 1. Node signs (must be ±1)
        // -------------------------------
        std::array<int, 4> s;
        for (int i = 0; i < 4; ++i)
        {
            s[i] = level_set_fields
        ...
        // -------------------------------
        // 2. Enrichment DOFs (a_i)
        // -------------------------------
        std::array<Eigen::Vector2d, 4> a;
        for (int i = 0; i < 4; ++i)
        {
            int off = node_offset[element[i]];
            a[i] = Eigen::Vector2d(u(off + 2), u(off + 3));
        }

        // -------------------------------
        // 3. Corner positions (ONLY standard displacement)
        // -------------------------------
        ...
        // -------------------------------
        // 5. Enriched position evaluator
        // -------------------------------
        auto enrichedPos = [&](double xi, double eta, int Hside)
            -> Eigen::Vector2d
        {
            auto Ni = N(xi, eta);

            // Base (standard FEM interpolation)
            Eigen::Vector2d base = Eigen::Vector2d::Zero();
            for (int i = 0; i < 4; ++i)
                base += Ni[i] * corners[i];

            // Enrichment
            Eigen::Vector2d enrich = Eigen::Vector2d::Zero();
            for (int i = 0; i < 4; ++i)
            {
                double shift = static_cast<double>(Hside - s[i]);
        ...
        // -------------------------------
        Eigen::Vector2d interPos[2], interNeg[2];
        ...
        for (int k = 0; k < 2; ++k)
        {
            ...
            std::cout << "  POS: " << interPos[k].transpose() << "\n";
            std::cout << "  NEG: " << interNeg[k].transpose() << "\n";
        }

        // -------------------------------
        // 7. Build point arrays
        // -------------------------------
        std::array<Eigen::Vector2d, 6> pointsPos, pointsNeg;

        for (int i = 0; i < 4; ++i)
        {
            // Nodes are continuous → same on both sides
            pointsPos[i] = corners[i];
            pointsNeg[i] = corners[i];
        }

        // Crack intersection points differ
            ...
            ++i)
        {
            ...
            TriangleGUI::Renderer::instance().addTriangle(
                TriangleGUI::TriangleColored{
                    toGlm(pointsPos[tri[0]].cast<float>().eval()),
                    toGlm(pointsPos[tri[1]].cast<float>().eval()),
                    toGlm(pointsPos[tri[2]].cast<float>().eval()),
                    packColor(glm::vec4(0.f, 1.f, 0.f, 1.f))});
        }

        // -------------------------------
        // 9. Draw negative side (blue)
        // -------------------------------
        for (unsigned int i =
            ...
            ++i)
        {
            ...
            TriangleGUI::Renderer::instance().addTriangle(
                TriangleGUI::TriangleColored{
                    toGlm(pointsNeg[tri[0]].cast<float>().eval()),
                    toGlm(pointsNeg[tri[1]].cast<float>().eval()),
                    toGlm(pointsNeg[tri[2]].cast<float>().eval()),
                    packColor(glm::vec4(0.f, 0.f, 1.f, 1.f))});
        }

        // -------------------------------
        // 10. Optional: draw element outline
        // -------------------------------
        quads.push_back(Quad{
            toGlm(corners[0].cast<float>().eval()),
            toGlm(corners[1].cast<float>().eval()),
            toGlm(corners[2].cast<float>().eval()),
            toGlm(corners[3].cast<float>().eval()),
            packColor(glm::vec4(1.f, 0.f, 0.f, 0.3f))});
    }
    unsigned int idx;
    for (int i = 0; i < wn; i++)
    {
