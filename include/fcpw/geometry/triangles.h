...
namespace fcpw {
inline bool pointInTriangle2D(const Vector2& p, const Vector2& a, const Vector2& b, const Vector2& c) {
    float s1 = (a.y() - c.y())*(p.x() - c.x()) + (c.x() - a.x())*(p.y() - c.y());
    float s2 = (b.y() - a.y())*(p.x() - a.x()) + (a.x() - b.x())*(p.y() - a.y());
    float s3 = (c.y() - b.y())*(p.x() - b.x()) + (b.x() - c.x())*(p.y() - b.y());
